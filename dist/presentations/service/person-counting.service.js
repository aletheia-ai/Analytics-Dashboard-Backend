        ...
        try {
            ...
            if (result) {
                ...
                if (!aggregatedResult) {
                    ...
                    const aggregatedAllResult = await this.stats.updateOne({ cameraId: 'all', range: range_type_1.RangeType.ALL_TIME }, {
                        $inc: incFields,
                        $set: {
                            ...setFields,
                            'data.cameraId': cameraId,
                    ...
                    if (!aggregatedAllResult) {
                        ...
                        if (statsData) {
                            ...
                            await this.dayWiseStats.updateOne({ day: dayName }, {
                                $inc: incFields,
                                $set: {
                                    ...setFields,
                                    'data.cameraId': cameraId,
                            ...
                            await this.hourWiseStats.updateOne({ hour: currentHour }, {
                                $inc: incFields,
                                $set: {
                                    ...setFields,
                                    'data.cameraId': cameraId,
    ...
    async getCurrentHourStats(store) {
        try {
            ...
            if (stats && stats.length > 0) {
                ...
                const { child, teen, middle_age, adult, old_age, enterCount } = (0, aggregated_stats_1.sumObjects)(result);
                return {
                    success: true,
                    data: {
                        child,
